...
def writeOutput(filename, algo, cutoff, random_seed, vertex_cover, trace_output):
    ...
    if random_seed is not None:
        sol_file = os.path.basename(filename)[:-6] + algo + str(cutoff) + "_" + str(random_seed) + ".sol" 
        trace_file = os.path.basename(filename)[:-6] + algo + str(cutoff) + "_" + str(random_seed) + ".trace" 
    else:
        sol_file = os.path.basename(filename)[:-6] + algo + str(cutoff) + ".sol" 
        trace_file = os.path.basename(filename)[:-6] + algo + str(cutoff) + ".trace" 
