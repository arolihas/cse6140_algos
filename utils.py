...
def writeOutput(filename, algo, cutoff, random_seed, vertex_cover, trace_output):
    ...
    sol_file = "./OutputBnB/" + os.path.basename(filename)[:-6] + algo + str(cutoff) + "_" + str(random_seed) + ".sol" 
    trace_file = "./OutputBnB/" +  os.path.basename(filename)[:-6] + algo + str(cutoff) + "_" + str(random_seed) + ".trace" 
